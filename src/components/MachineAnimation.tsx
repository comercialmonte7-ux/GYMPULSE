...
const TechnicalAvatar: React.FC<{ type: string; progress: any }> = ({ type, progress }) => {
  // Simplified joint-based model
  return (
    <g className="stroke-slate-900 stroke-[1.5] fill-none">
      {type === 'squat' ? (
        <motion.g animate={{ y: progress.y }}>
          {/* Trunk */}
          <path d="M50 35 L50 60" />
          {/* Head */}
          <circle cx="50" cy="28" r="6" className="fill-slate-50" />
          {/* Thighs */}
          <motion.path 
            d="M50 60 L35 75" 
        ...
        <g>
          <path d="M40 30 L40 60" className="opacity-20" strokeWidth="4" /> {/* Bench */}
          <circle cx="45" cy="25" r="5" /> {/* Head */}
          <path d="M45 30 L45 55" /> {/* Torso */}
          <motion.path 
            d="M45 40 L60 40 L70 40" 
            animate={{ x: [0, 15, 0] }}
            transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
          />
          <path d="M45 55 L60 65" /> {/* Leg */}
        </g>
      ) : type === 'pull' ? (
        <g>
          <path d="M50 20 L50 80" className="opacity-10" strokeWidth="8" /> {/* Machine Column */}
          <circle cx="40" cy="35" r="5" /> {/* Head */}
          <path d="M40 40 L40 65" /> {/* Torso */}
          <motion.path 
        ...
        <g>
          <circle cx="50" cy="30" r="6" /> {/* Head */}
          <path d="M50 36 L50 60" /> {/* Torso */}
...
export const MachineAnimation: React.FC<MachineAnimationProps> = ({ machineId, videoUrl }) => {
  ...
  return (
    <div className="w-full h-full bg-white rounded-[2.5rem] flex flex-col items-center justify-center relative overflow-hidden group shadow-[inset_0_2px_10px_rgba(0,0,0,0.05)] border border-slate-200/60">
      ...
      {videoUrl && showVideo ? (
        <div className="absolute inset-0 w-full h-full bg-black z-20">
          <iframe 
            ...
            src={`https://www.youtube-nocookie.com/embed/${videoUrl}?rel=0&autoplay=1&mute=1&playlist=${videoUrl}&loop=1`}
        ...
        <div className="w-full h-full flex flex-col items-center justify-center p-8 relative">
            ...
            <div className="absolute top-8 left-8 space-y-1">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Analysis Mode</span>
                </div>
                <div className="text-[9px] font-mono text-slate-300">ID_REF: {machineId.toUpperCase()}</div>
            </div>
            ...
            <div className="relative w-full max-w-[280px] h-full flex items-center justify-center">
                <svg viewBox="0 0 100 100" className="w-full h-full overflow-visible">
                    ...
                    {(isPress || isPull) && (
                        <g opacity="0.1">
                           <rect x="20" y="20" width="10" height="70" rx="2" fill="currentColor" className="text-slate-900" />
                           <path d="M25 20 L80 20" stroke="currentColor" strokeDasharray="2 2" className="text-slate-900" />
                        </g>
                    )}
                    ...
                    <motion.g
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        className="stroke-indigo-500 stroke-[1] fill-none"
                    >
                        <motion.path 
                    ...
                    {/* Callouts */}
                    <g className="text-[4px] font-mono fill-indigo-400 font-bold uppercase">
                ...
                <div className="absolute bottom-4 right-0 flex flex-col items-end">
                    <div className="bg-slate-900 text-white text-[8px] font-mono px-2 py-0.5 rounded-sm flex items-center gap-1.5 mb-1">
                        <span className="w-1 h-1 bg-emerald-400 rounded-full animate-pulse" />
                        TRACKING_OK
                    </div>
                    <div className="text-[10px] font-black text-slate-900 tabular-nums uppercase tracking-tighter">
      ...
      <div className="absolute top-6 right-6 flex gap-2 z-30">
        {videoUrl && (
            ...
            >
                {showVideo ? 'Ver Esquema Técnico' : 'Ver Guía Pro (Video)'}
            </button>
        )}
      </div>
      ...
      <div className="absolute bottom-6 left-8 flex items-center gap-3 z-30">
        <div className="flex gap-0.5">
            {[1,2,3].map(i => <div key={i} className="w-1 h-3 bg-indigo-500/20 rounded-full" />)}
        </div>
